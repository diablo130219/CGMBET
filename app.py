...
dettagli = {
    ...
    "quota": {"val": float(m["odd"] or 0), "ok": ok_q,
              "soglia": f'{s["quota_min"]}–{s["quota_max"]}'},
}
...
for combo in combos:
    ...
    for idx in combo:
        ...
        nomi.append(f"{partite[idx]['home_team']} - {partite[idx]['away_team']}")
    triple.append({"partite": nomi, "quota": round(q, 2)})

inv_sum = sum(1 / t["quota"] for t in triple if t["quota"] > 0)
for t in triple:
    t["puntata"] = round(budget * (1 / t["quota"]) / inv_sum, 2) if inv_sum > 0 else 0
